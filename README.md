...
- 中文界面，第一屏直接进入计时器。
- 普通模式采用左侧计时主控台、右侧设置检查器布局，开始、重置、全屏和声音控制集中在主计时面板内。
- 默认标题为“现场计时”，支持自定义标题、小时/分钟/秒、自定义提醒节点和快捷时长。
- 支持演讲模式、课堂展示、答辩模式、会议发言、自由计时等场景预设，一次套用时长、提醒、超时和全屏显示设置。
- 支持开始、暂停、继续、运行中二次确认重置和大屏展示模式。
- 大屏展示模式显示标题、大号时间、已进行时长和剩余时长，可在右上角弱显示真实时间，并可开启类似 iPhone 横屏 StandBy 计时器的全屏进度展示。
...
- 自动保存标题、总时长、提醒节点、提示音、超时、全屏真实时间和全屏进度条设置，刷新后不会自动开始计时。
